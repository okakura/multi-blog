...
import { useAdminDomains } from '@/data/hooks/useAdminPosts'
import type {
  CreateDomainRequest,
  Domain,
  UpdateDomainRequest,
                      ...
                      <div className='flex flex-wrap gap-1'>
                        {domain.categories
                          ...
                          .map((category, index) => (
                            <span
                              key={index}
                              className='inline-block bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 px-2 py-1 rounded text-xs'>
                              {category}
                            </span>
                          ))}
                        {domain.categories.length > 3 && (
                          <span className='inline-block text-slate-400 dark:text-slate-500 px-2 py-1 text-xs'>
