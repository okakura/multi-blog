...
const AdminDashboard = () => {
      ...
      fetch('http://localhost:8000/auth/login', {
        ...
        .then((data) => {
          if (data.token) {
            ...
            console.log('Development JWT token set')
          }
        })
        .catch((error) => {
  ...
  const getDomainPerformance = () => {
    ...
    const domainStats = recentPosts.reduce(
      (acc, post) => {
  ...
  const stats = [
    ...
    {
      title: 'Monthly Views',
      value: analytics?.views_this_month
        ? analytics.views_this_month > 1000
          ? `${(analytics.views_this_month / 1000).toFixed(1)}K`
          : analytics.views_this_month.toString()
        : '0',
      change:
        analytics?.views_this_month && analytics.views_this_month > 50
          ? '+18%'
          : analytics?.views_this_month && analytics.views_this_month > 10
            ? '+8%'
            : '+2%',
      ...
      color: 'bg-purple-500',
      trend:
        analytics?.views_this_month && analytics.views_this_month > 0
          ? 'up'
          : 'neutral',
    },
    {
      title: 'Unique Visitors',
      value: analytics?.total_views?.toString() || '0',
      change:
        analytics?.total_views && analytics.total_views > 3
          ? '+15%'
          : analytics?.total_views && analytics.total_views > 1
            ? '+5%'
            : '+0%',
      changeType:
        analytics?.total_views && analytics.total_views > 1
      ...
      color: 'bg-orange-500',
      trend:
        analytics?.total_views && analytics.total_views > 1 ? 'up' : 'neutral',
    },
  ]
              ...
              {recentPosts.length > 0 ? (
                recentPosts.slice(0, 5).map((post) => (
                  <button
                    key={post.id}
                    type="button"
                    className="w-full text-left p-6 hover:bg-slate-50 dark:hover:bg-gray-700 transition-colors cursor-pointer group"
              ...
              <div className="space-y-4">
                {topDomains.map((domain, index) => (
                  <button
                    key={domain.name}
                    type="button"
                    className="w-full text-left flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 dark:hover:bg-gray-700 transition-colors cursor-pointer group"
                    onClick={() =>
