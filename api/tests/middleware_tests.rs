...
use api::{
    AppState, DomainContext, analytics_middleware, auth_middleware, domain_middleware,
    test_utils::*,
};
use axum::{
    Extension, Router,
...
async fn test_analytics_handler(Extension(analytics): Extension<api::AnalyticsContext>) -> String {
    format!(
        "Analytics: {} from {}",
        analytics.ip_address, analytics.user_agent
    )
}
