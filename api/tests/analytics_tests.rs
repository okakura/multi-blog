...
use api::{handlers::analytics::AnalyticsModule, test_utils::*, AppState, DomainContext, UserContext};
...
async fn create_test_analytics_data(pool: &sqlx::PgPool, domain_id: i32, post_id: Option<i32>) {
    let events = vec![
        ("page_view", "/", "127.0.0.1", "Mozilla/5.0", Some("https://google.com")),
        ("post_view", "/posts/test", "127.0.0.2", "Chrome/91.0", Some("https://facebook.com")),
...
async fn test_search_analytics() {
    ...
    let rust_term = popular_terms.iter()
...
async fn test_referrer_stats() {
    ...
    assert!(referrer_types.get("search_engines").unwrap().as_i64().unwrap() > 0);
    assert!(referrer_types.get("social_media").unwrap().as_i64().unwrap() > 0);
    assert!(referrer_types.get("other_websites").unwrap().as_i64().unwrap() > 0);
...
async fn test_post_analytics() {
    ...
    let post = &posts[0];
    assert_eq!(post.get("title").unwrap().as_str().unwrap(), "Analytics Post");
